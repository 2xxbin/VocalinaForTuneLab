...
> 이펙트 아이디어 구체화, 코드 리팩토링, SDK 구조 이해 등에 사용되었습니다. <br>
> 모든 AI 생성 코드는 사람이 재검토 및 수정하여 사용하였습니다.
...
> ~~2.2 버전에서는 인토네이션 같은 피치 렌더링 기능이 없어 제외되었습니다.~~ <br>
> 2026-08-09: 외부 이펙트 방식을 사용하여 구현하였습니다.

3. 일본어 및 중국어 문자와 한국어 로마자를 사용할 수 없습니다.

4. `TuneLab` 실행 속도가 조금 느려질 수 있습니다.

5. `Vocalina 2.2`에서 피치 조교 및 파라미터를 사용 시, 합성 속도가 느려집니다.
> 이펙트 구현 방식으로 인한 한계 사항입니다.
...
4. [`VocalinaForTuneLab` 릴리즈 페이지](https://github.com/2xxbin/VocalinaForTuneLab/releases)에서 **본인이 사용할 보카리나 버전의 확장을** 다운로드합니다.
5. 실행된 `TuneLab`에 `.tlx` 파일을 드래그해 보카리나 확장을 설치합니다.
6. `TuneLab`을 종료 후 재실행하여 보이스 목록에 `Vocalina 2.X`가 뜨는지 확인합니다.
7. 정상적으로 동작하는지 테스트합니다.

만약 본인이 보카리나 설치 폴더가 기본 경로가 아닐 시, 설정 수정이 필요합니다.

1. `TuneLab`을 실행한 뒤, 오른쪽 아래 톱나바퀴를 눌러 설정화면으로 접근합니다.
2. 왼쪽 사이드바의 `확장`을 선택합니다.
3. `VOCALINA 2.2` 혹은 `VOCALINA 2.3` 타이틀의 `Vocalina Install Path`의 경로를 보카리나 경로에 맞게 수정합니다.
4. `TuneLab`을 재실행하여 테스트해봅니다.
...
1. `TuneLab`을 실행한 뒤, 오른쪽 아래 톱나바퀴를 눌러 설정화면으로 접근합니다.
2. 왼쪽 사이드바의 `확장`을 선택합니다.
3. `VOCALINA 2.2` 혹은 `VOCALINA 2.3` 타이틀의 `Show Render Server Console Window`의 체크박스를 활성화합니다.
4. `TuneLab`을 재실행해 검은 터미널 창이 뜨는지 확인해주세요.
5. 문제를 재현한 뒤, 검은 터미널 창에 뜬 내용을 확인해주세요.
